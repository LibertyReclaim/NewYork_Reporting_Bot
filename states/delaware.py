...
TARGET_URL = "https://unclaimedproperty.delaware.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[DE][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[DE][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[DE] Dropdown '{label}' value is blank")
...
def safe_select_date_triplet(page: Page, label: str, mm: str, dd: str, yyyy: str, optional: bool = False) -> None:
    ...
    def select_part(sel: Locator, val: str, part_name: str) -> None:
        ...
        for i in range(options.count()):
            ...
            if label_text == val or value_text == val:
                sel.first.select_option(value=value_text or label_text, timeout=10_000)
                return
        raise RuntimeError(f"[DE] Could not set {label} {part_name}={val}")

    select_part(mm_sel, mm, "MM")
    select_part(dd_sel, dd, "DD")
    select_part(yy_sel, yyyy, "YYYY")
...
def click_next(page: Page, step_name: str) -> None:
    ...
    if not locator:
        raise RuntimeError("[DE] Could not find NEXT button")
    log_debug(f"NEXT strategy={strategy}")
    locator.scroll_into_view_if_needed(timeout=10_000)
    locator.click(timeout=15_000)


def run_delaware(context, company_data: dict, filing_data: dict) -> dict:
    ...
    log_debug("Holder ID should remain blank for Delaware; skipping")
    ...
    # Optional reporting agent fields
    safe_fill_by_label(page, "Reporting Agent Name", str(company_data.get("reporting_agent_name", "")).strip(), optional=True)
    safe_fill_by_label(page, "Reporting Agent FEIN", str(company_data.get("reporting_agent_fein", "")).strip(), optional=True)
    ...
    postal = str(company_data.get("zip") or company_data.get("zip_code") or "").strip()
    safe_fill_by_label(page, "Postal Code", postal)

    safe_select_by_label(page, "State of Incorporation", str(company_data.get("state_of_incorporation", "")).strip(), optional=True)

    log_debug("Filling Date of Incorporation: 1 / 1 / 2026")
    safe_select_date_triplet(page, "Date of Incorporation", "1", "1", "2026", optional=True)
    ...
    if negative_report:
        log_debug("Negative Report is Yes; skipping disabled report totals fields")
    else:
        total_cash = filing_data.get("total_amount_of_cash_reported") or filing_data.get("total_cash_reported") or ""
        safe_fill_by_label(page, "Total Amount of Cash Reported", normalize_number(total_cash, default=""), optional=True)

        total_remitted = normalize_number(filing_data.get("total_dollar_amount_remitted"), default="")
        log_debug(f"Filling Total Dollar Amount Remitted: {total_remitted!r}")
        safe_fill_by_label(page, "Total Dollar Amount Remitted", total_remitted, optional=True)

        total_shares = filing_data.get("total_number_of_shares_reported") or filing_data.get("total_shares_reported") or ""
        safe_fill_by_label(page, "Total Number of Shares Reported", normalize_number(total_shares, default=""), optional=True)

        log_debug("Selecting How are you Remitting Property: 'Check'")
        safe_select_by_label(page, "How are you Remitting Property", "Check", optional=True)

    safe_fill_by_label(
        page,
        "Total Number of Owners Reported",
        normalize_number(filing_data.get("total_number_of_owners_reported"), default=""),
        optional=True,
    )
    safe_fill_by_label(
        page,
        "Total Number of Properties Reported",
        normalize_number(filing_data.get("total_number_of_properties_reported"), default=""),
        optional=True,
    )
    ...
    click_next(page, "after DE holder info")
    ...
    log_step("Reached Delaware upload step.")
