...
TARGET_URL = "https://nvup.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[NV][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[NV][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[NV] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[NV] Could not set radio '{group_label}' to {target}")
...
def select_funds_online(page: Page) -> None:
    ...
    if not matched:
        raise RuntimeError("[NV] Could not set Funds Remitted Via to 'Online'")
...
def run_nevada(context, company_data: dict, filing_data: dict) -> dict:
