...
TARGET_URL = "https://greatiowatreasurehunt.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[IA][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[IA][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[IA] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[IA] Could not set radio '{group_label}' to {target}")
...
def run_iowa(context, company_data: dict, filing_data: dict) -> dict:
    ...
    safe_fill_by_label(page, "Tax ID", str(company_data.get("holder_tax_id", "")).strip())
    safe_fill_by_label(page, "Holder ID", str(company_data.get("holder_id", "")).strip(), optional=True)
    safe_fill_by_label(page, "Contact", str(company_data.get("contact_name", "")).strip())
    ...
    log_step("Reached Iowa upload step.")
