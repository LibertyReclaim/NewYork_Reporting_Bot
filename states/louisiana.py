...
TARGET_URL = "https://unclaimedproperty.la.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[LA][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[LA][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[LA] Dropdown '{label}' value is blank")
    ...
    for i in range(options.count()):
        ...
        if opt_label == value_str or opt_value == value_str:
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[LA] Could not set radio '{group_label}' to {target}")


def select_state_of_incorporation_louisiana(page: Page) -> None:
    log_debug("Selecting State of Incorporation: Louisiana")
    found = get_field_locator(page, "State of Incorporation", kind="select")
    if not found:
        raise RuntimeError("[LA] Dropdown not found: State of Incorporation")
    _, locator = found
    if not locator:
        raise RuntimeError("[LA] Dropdown not found: State of Incorporation")

    if is_disabled_or_readonly(locator):
        log_debug("State of Incorporation dropdown is disabled/read-only; skipping")
        return

    locator.scroll_into_view_if_needed(timeout=10_000)
    try:
        locator.select_option(label="Louisiana", timeout=10_000)
        return
    except Exception:
        pass
    ...
    raise RuntimeError("[LA] Could not set State of Incorporation to Louisiana")
...
def run_louisiana(context, company_data: dict, filing_data: dict) -> dict:
    page = context.new_page()

    log_step("Navigating to holder page")
    ...
    safe_fill_by_label(page, "Federal ID Number", str(company_data.get("holder_tax_id", "")).strip())

    log_debug("Holder ID should remain blank for Louisiana; skipping")

    safe_fill_by_label(page, "Contact Person", str(company_data.get("contact_name", "")).strip())
    safe_fill_by_label(page, "Telephone Number", str(company_data.get("contact_phone", "")).strip())
    ...
    select_state_of_incorporation_louisiana(page)
    ...
    if not negative_report:
        log_debug("Filling Total Dollar Amount Remitted")
        safe_fill_by_label(
            page,
        ...
        shares = filing_data.get("number_of_shares_reported")
        if shares is not None and str(shares).strip() != "":
            safe_fill_by_label(page, "Number of Shares Reported", normalize_number(shares, default=""), optional=True)
    ...
    log_step("Reached Louisiana upload step.")
