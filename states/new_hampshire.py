...
TARGET_URL = "https://findnhmoney.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[NH][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[NH][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[NH] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[NH] Could not set radio '{group_label}' to {target}")
...
def run_new_hampshire(context, company_data: dict, filing_data: dict) -> dict:
    ...
    log_step("Filling Primary Holder Info")
    ...
    safe_fill_by_label(page, "Email Address Confirmation", str(company_data.get("email_confirmation", "")).strip())

    log_step("Filling Address Section")
    ...
    safe_select_by_label(page, "State", "New Hampshire")
    safe_fill_by_label(page, "Zip Code", str(company_data.get("zip", "")).strip())

    safe_select_by_label(page, "State of Incorporation", str(company_data.get("state", "")).strip(), optional=True)

    log_step("Selecting Date of Incorporation triplet")
    ...
    safe_check_radio(page, "Negative Report", normalize_bool(filing_data.get("negative_report")), optional=True)
    safe_fill_by_label(
        page,
    ...
    safe_select_by_label(page, "Funds Remitted Via", "Wire")

    click_next(page)
    print("NH reached_upload_step")
