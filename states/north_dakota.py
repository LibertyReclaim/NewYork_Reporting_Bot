...
TARGET_URL = "https://unclaimedproperty.nd.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[ND][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[ND][DEBUG] {message}")
...
def safe_fill_by_label(page: Page, label: str, value: str, optional: bool = False) -> None:
    ...
    if not found:
        if optional:
            ...
            return
        raise RuntimeError(f"[ND] Field not found: {label}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not found:
        if optional:
            ...
            return False
        raise RuntimeError(f"[ND] Dropdown not found: {label}")
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[ND] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if question.count() == 0:
        if optional:
            ...
            return
        raise RuntimeError(f"[ND] Radio group not found: {group_label}")
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[ND] Could not set radio '{group_label}' to {target}")
...
def run_north_dakota(context, company_data: dict, filing_data: dict) -> dict:
    ...
    safe_fill_by_label(page, "NAICS Code", str(company_data.get("naics_code", "")).strip(), optional=True)
    ...
    # Report Information
    safe_select_by_label(page, "Report Type", "Annual")
    safe_select_by_label(page, "Report Year", str(filing_data.get("report_year", "")).strip(), optional=True)
    safe_check_radio(page, "This is a Zero/Negative Report", False, optional=True)
    safe_fill_by_label(
        page,
    ...
    safe_select_by_label(page, "Funds Remitted Via", "Check", optional=False)

    click_next(page)
    print("ND reached_upload_step")
