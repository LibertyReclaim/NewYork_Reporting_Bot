...
TARGET_URL = "https://unclaimed.nccash.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[NC][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[NC][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[NC] Dropdown '{label}' value is blank")
...
def select_nc_funds_remitted_check(page: Page) -> None:
    log_debug("Selecting Funds Remitted Via: 'Check'")
    found = get_field_locator(page, "Funds Remitted Via", kind="select")
    if not found:
        raise RuntimeError("[NC] Dropdown not found: Funds Remitted Via")
    _, locator = found
    if not locator:
        raise RuntimeError("[NC] Dropdown not found: Funds Remitted Via")
    ...
    locator.scroll_into_view_if_needed(timeout=10_000)
    try:
        locator.select_option(label="Check", timeout=10_000)
        return
    except Exception:
        pass
    ...
    raise RuntimeError("[NC] Could not set Funds Remitted Via to 'Check'")
...
def click_next(page: Page, step_name: str) -> None:
    ...
    if not locator:
        raise RuntimeError("[NC] Could not find NEXT button")
    log_debug(f"NEXT strategy={strategy}")
    locator.scroll_into_view_if_needed(timeout=10_000)
    locator.click(timeout=15_000)


def run_north_carolina(context, company_data: dict, filing_data: dict) -> dict:
    ...
    log_debug("Holder ID should remain blank for North Carolina; skipping")
    ...
    if not negative_report:
        ...
        select_nc_funds_remitted_check(page)
    ...
    click_next(page, "after NC holder info")
    ...
    log_step("Reached North Carolina upload step.")
