...
TARGET_URL = "https://maineunclaimedproperty.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[ME][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[ME][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[ME] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[ME] Could not set radio '{group_label}' to {target}")


def run_maine(context, company_data: dict, filing_data: dict) -> dict:
    ...
    log_debug("Skipping d/b/a Name (not in workbook mapping)")
    ...
    safe_fill_by_label(page, "Contact Title", "Owner")
    ...
    safe_fill_by_label(page, "Email Confirmation", str(company_data.get("email_confirmation", "")).strip())

    # Address Section
    safe_fill_by_label(page, "Mailing Address", str(company_data.get("address_1", "")).strip())
    safe_fill_by_label(page, "City", str(company_data.get("city", "")).strip())
    log_debug("Leaving State default as Maine")
    safe_fill_by_label(page, "Zip", str(company_data.get("zip", "")).strip())

    # Business Info
    safe_select_by_label(page, "State of Incorporation", "Maine")
    safe_check_radio(page, "Does the Report Include All Branches and Divisions", False)
    safe_check_radio(page, "Does the Report Include All Subsidiaries", False)
    safe_fill_by_label(page, "Type of Business", "Other")
    ...
    # Report Totals
    safe_fill_by_label(page, "Total Number of Items Reported", "1")
    safe_fill_by_label(page, "Safekeeping Items", "0")
    safe_fill_by_label(page, "Shares of Stocks or Mutual Funds Remitted", "0")
    safe_fill_by_label(
        page,
    ...
    safe_check_radio(
        page,
        "Does this report include records subject to HIPAA",
