...
TARGET_URL = "https://yourmoney.idaho.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[ID][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[ID][DEBUG] {message}")
...
def safe_fill_by_label(page: Page, label: str, value: str, optional: bool = False) -> None:
    ...
    if not found:
        if optional:
            ...
            return
        raise RuntimeError(f"[ID] Field not found: {label}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not found:
        if optional:
            ...
            return False
        raise RuntimeError(f"[ID] Dropdown not found: {label}")
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[ID] Dropdown '{label}' value is blank")
...
def select_zero_report_no(page: Page) -> None:
    question = page.get_by_text("This is a Zero Report", exact=False)
    if question.count() == 0:
        log_debug("Zero Report question not found; skipping")
        return
...
def select_funds_online(page: Page) -> None:
    log_debug("Selecting Funds Remitted Via: 'Online'")
    if safe_select_by_label(page, "Funds Remitted Via", "Online", optional=True):
        return
    raise RuntimeError("[ID] Could not set Funds Remitted Via to 'Online'")
...
def run_idaho(context, company_data: dict, filing_data: dict) -> dict:
    ...
    safe_fill_by_label(page, "FEIN", str(company_data.get("holder_tax_id", "")).strip())
    ...
    safe_select_by_label(page, "State", "Idaho")
