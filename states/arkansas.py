...
TARGET_URL = "https://claimitar.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[AR][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[AR][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[AR] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[AR] Could not set radio '{group_label}' to {target}")
...
def run_arkansas(context, company_data: dict, filing_data: dict) -> dict:
    ...
    log_step("Reached Arkansas upload step.")
