...
TARGET_URL = "https://missingmoney.ks.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[KS][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[KS][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[KS] Dropdown '{label}' value is blank")
    ...
    # fallback loop + visible text match
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[KS] Could not set radio '{group_label}' to {target}")
...
def select_funds_electronic(page: Page) -> None:
    ...
    if not found:
        raise RuntimeError("[KS] Dropdown not found: Funds Remitted Via")
    _, locator = found
    if not locator:
        raise RuntimeError("[KS] Dropdown not found: Funds Remitted Via")
    ...
    raise RuntimeError("[KS] Could not set Funds Remitted Via to 'Electronic'")
...
def run_kansas(context, company_data: dict, filing_data: dict) -> dict:
    ...
    log_step("Reached Kansas upload step.")
