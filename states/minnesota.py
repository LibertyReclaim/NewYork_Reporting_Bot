...
TARGET_URL = "https://minnesota.findyourunclaimedproperty.com/app/holder-info"


def log_step(message: str) -> None:
    print(f"[MN][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[MN][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[MN] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[MN] Could not set radio '{group_label}' to {target}")


def select_funds_online(page: Page) -> None:
    log_debug("Selecting Funds Remitted Via: 'Online'")
    matched = safe_select_by_label(page, "Funds Remitted Via", "Online")
    if not matched:
        raise RuntimeError("[MN] Could not set Funds Remitted Via to 'Online'")
...
def run_minnesota(context, company_data: dict, filing_data: dict) -> dict:
    ...
    # Funds Remitted Via (must be Online)
    select_funds_online(page)

    click_next(page)

    print("MN reached_upload_step")
