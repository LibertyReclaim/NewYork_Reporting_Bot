...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[SC] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    container_label = q.locator("xpath=ancestor::*[self::div or self::form][1]").get_by_text(target, exact=True)
    if container_label.count() > 0 and container_label.first.is_visible():
        container_label.first.scroll_into_view_if_needed(timeout=10_000)
        container_label.first.click(timeout=10_000)
        log_debug(f"Radio '{group_label}' set to {target}")
        return
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[SC] Could not set radio '{group_label}' to {target}")


def select_sc_funds_wire(page: Page) -> None:
    log_debug("Selecting Funds Remitted Via: 'Wire'")
    found = get_field_locator(page, "Funds Remitted Via", kind="select")
    if not found:
        raise RuntimeError("[SC] Dropdown not found: Funds Remitted Via")
    _, locator = found
    if not locator:
        raise RuntimeError("[SC] Dropdown not found: Funds Remitted Via")
    ...
    locator.scroll_into_view_if_needed(timeout=10_000)
    try:
        locator.select_option(label="Wire", timeout=10_000)
        return
    except Exception:
        pass
    ...
    raise RuntimeError("[SC] Could not set Funds Remitted Via to 'Wire'")
...
def run_south_carolina(context, company_data: dict, filing_data: dict) -> dict:
    ...
    log_debug("Holder ID should remain blank for South Carolina; skipping")
    ...
    if not negative_report:
        ...
        safe_fill_by_label(page, "Total Dollar Amount Remitted", remitted, optional=True)
        select_sc_funds_wire(page)
