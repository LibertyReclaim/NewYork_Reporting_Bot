...
TARGET_URL = "https://unclaimed.alabama.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[AL][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[AL][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[AL] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[AL] Could not set radio '{group_label}' to {target}")
...
def run_alabama(context, company_data: dict, filing_data: dict) -> dict:
    page = context.new_page()

    log_step(f"Navigating to {TARGET_URL}")
    page.goto(TARGET_URL, wait_until="domcontentloaded", timeout=60_000)
    page.wait_for_load_state("networkidle", timeout=60_000)
    page.get_by_label("Business Name", exact=False).first.wait_for(state="visible", timeout=30_000)

    # Primary Business Info
    safe_fill_by_label(page, "Business Name", str(company_data.get("holder_name", "")).strip())
    safe_fill_by_label(page, "Business Tax ID", str(company_data.get("holder_tax_id", "")).strip())

    log_debug("Holder ID should remain blank for Alabama; skipping")

    safe_fill_by_label(page, "Business Contact", str(company_data.get("contact_name", "")).strip())
    ...
    # State may already default to Alabama, set only if editable.
    ...
    negative = normalize_bool(filing_data.get("negative_report"))
    safe_check_radio(page, "This is a Negative Report", negative, optional=True)
    ...
    log_step("Reached Alabama upload step.")
