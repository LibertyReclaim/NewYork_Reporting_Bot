...
TARGET_URL = "https://unclaimedproperty.michigan.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[MI][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[MI][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[MI] Dropdown '{label}' value is blank")
...
def click_next(page: Page, step_name: str) -> None:
    ...
    if not locator:
        raise RuntimeError("[MI] Could not find NEXT button")
    log_debug(f"NEXT strategy={strategy}")
    locator.scroll_into_view_if_needed(timeout=10_000)
    locator.click(timeout=15_000)


def run_michigan(context, company_data: dict, filing_data: dict) -> dict:
    ...
    log_step("Reached Michigan upload step.")
