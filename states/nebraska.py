...
TARGET_URL = "https://nebraskalostcash.nebraska.gov/app/holder-info"


def log_step(message: str) -> None:
    print(f"[NE][STEP] {message}")


def log_debug(message: str) -> None:
    print(f"[NE][DEBUG] {message}")
...
def safe_select_by_label(page: Page, label: str, value: str, optional: bool = False) -> bool:
    ...
    if not value_str:
        if optional:
            ...
            return False
        raise RuntimeError(f"[NE] Dropdown '{label}' value is blank")
...
def safe_check_radio(page: Page, group_label: str, yes_value: bool, optional: bool = False) -> None:
    ...
    if optional:
        ...
        return
    raise RuntimeError(f"[NE] Could not set radio '{group_label}' to {target}")
...
def select_funds_online(page: Page) -> None:
    ...
    if not matched:
        raise RuntimeError("[NE] Could not set Funds Remitted Via to 'Online'")
...
def run_nebraska(context, company_data: dict, filing_data: dict) -> dict:
