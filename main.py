        ...
        for state_code in requested_states:
            ...
            try:
                ...
                if state_code == "NY":
                    ...
                    result = run_colorado(context=context, company_data=company_data, filing_data=filing_data)
                    summary[state_code] = result.get("status", str(result)) if isinstance(result, dict) else str(result)
                elif state_code == "TX":
                    result = run_texas(
                        context=context,
                    ...
                    result = run_mississippi(context, company_data, filing_data)
                    summary[state_code] = result.get("status", str(result)) if isinstance(result, dict) else str(result)
                elif state_code == "IN":
                    result = run_indiana(
                        context=context,
                    ...
                    result = run_north_dakota(context, company_data, filing_data)
                    summary[state_code] = result.get("status", str(result)) if isinstance(result, dict) else str(result)
                elif state_code == "SC":
                    result = run_south_carolina(
                        context=context,
                        company_data=company_data,
                        filing_data=filing_data,
                    )
